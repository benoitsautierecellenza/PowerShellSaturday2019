...
# . .\AFW-ProcessRules.PS1 -ResourceName "finaltest7" -ServiceType "Storage" -OperationName "Create" -SourceAddress $Parameters
# Rendre le paramètre $SourceAddress obligatoire uniquement si Create
#
# Prochaine étape, processer que Azure Queue en entrée
# On parse les messages pour chaque classe (queue par service)
# on les traite en séquence pour l'instant
#
[OutputType("String")]
Param(
    [Parameter (Mandatory=$False)]
    [String]$ResourceName = "finaltest7",
...
[Int]$LockPauseDuration = 30
[String]$AFWLockName = "AFW"
#
# Variables
#
...
#try {
    ...
    ForEach ($AzureFirewall in $AzureFirewallList) {
        ...
        If ($CheckForLock.Count -GT 0)
        {
            ...
            #
            While ($CheckForLock.Count -gt 0)
            {
                Write-Warning "[AzureFirewall] - Concurrent Azure Firewall operation in process on $($AzureFirewall.name), waiting for lock removal."
                Start-Sleep -Seconds $LockPauseDuration
                $CheckForLock = Get-AzResourceLock -ResourceGroupName $AzureFirewall.resourcegroupname -ResourceName $AzureFirewall.name -ResourceType "Microsoft.Network/azureFirewalls"
            }
            Write-Warning "[AzureFirewall] - Concurrent Azure Firewall operation now completed on $($AzureFirewall.name)."            
        }
        else {
            Write-Host "[AzureFirewall] - No Azure concurrent azure Firewall operation in progress on $($AzureFirewall.name). Locking Azure Firewall Resource"
        }
        $Lock = New-AzResourceLock -ResourceGroupName $AzureFirewall.resourcegroupname `
            -ResourceName $($AzureFirewall.name) `
            -LockName $AFWLockName `
            -LockLevel CanNotDelete  `
            -ResourceType "Microsoft.Network/azureFirewalls" `
            -LockNotes "Resource Locked because Azure Firewall update is on progress" `
            -Force
        #
